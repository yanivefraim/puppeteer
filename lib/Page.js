...
class Page extends EventEmitter {
  ...
  _matchUrl(url) {
    if (url instanceof RegExp) {
      const keys = this._networkManager._requestUrlToRequest.keys();
      while (!keys.next().done) {
        if (url.exec(keys.next().value))
          return true;
      }
      return false;
    }
    return this._networkManager._requestUrlToRequest.get(url);
  }
  ...
  async waitForResponse(url) {
    return new Promise((resolve, reject) => {
      if (this._matchUrl(url)) {
        ...
        this._networkManager.on(NetworkManager.Events.Response, response => {
          if ((url instanceof RegExp && url.exec(response.url())) || response.url() === url)
            resolve();
        });
      }
    });
